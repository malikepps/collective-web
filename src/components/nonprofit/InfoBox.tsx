...
import ThemeSelectionSheet from '../theme/ThemeSelectionSheet';
...
interface InfoBoxProps {
  ...
  isUserStaff?: boolean;
  onOrganizationUpdate?: (updatedData: Partial<Organization>) => void;
}
...
const InfoBox: React.FC<InfoBoxProps> = ({
  ...
  // --- State for Theme Selection Sheet ---
  const [showThemeSheet, setShowThemeSheet] = useState(false); // Add state for sheet visibility
  ...
  return (
    <>
          ...
          {isUserStaff ? (
            ...
            <div className="flex flex-col space-y-2"> 
              ...
              <div className="flex space-x-2"> 
                ...
                {/* Theme Picker Button - Updated onClick */}
                <button
                  onClick={handleOpenThemeSheet} // Open the theme sheet
                  style={{ color: theme?.secondaryColor ? `#${theme.secondaryColor}` : '#8BBEF9' }}
      ...
      {/* Render ThemeSelectionSheet - Uncommented and props passed */}
      {showThemeSheet && orgId && (
        <ThemeSelectionSheet 
          isOpen={showThemeSheet}
          onClose={handleCloseThemeSheet}
          onSave={handleSaveTheme}
          organizationId={orgId}
          initialThemeId={organization.themeId || null}
        />
      )}
    </>
  );
